...
struct MatrixBlock
{
    ...
    double *data;  // one-dimensional array to store matrix entries
};

void blockedMatrixMultiplyAdd(
    int n, int blockSize,
    const MatrixBlock *X, const MatrixBlock *Y, MatrixBlock *Z)
{

    // Iterate over blocks (i, j) of the result matrix Z
    for (int i = 0; i < n; i += blockSize)
    {
        for (int j = 0; j < n; j += blockSize)
        {

            // Iterate over blocks (i, k) of X and (k, j) of Y for multiplication
            for (int k = 0; k < n; k += blockSize)
            {

                // Perform block-by-block multiplication and addition
